...
describe("Post's", () => {
    ...
    it("should update category teaser properly", async () => {
        ...
        assert.equal(
            data.categories[0].posts[0].pid,
            newPostResult.postData.pid,
        );
    ...
    it("should change owner of post and topic properly", async () => {
        ...
        assert.deepStrictEqual(
            await db.sortedSetScores(
                `tid:${postResult.topicData.tid}:posters`,
                [oldUid, newUid],
            ),
            [2, null],
        );
        ...
        assert.strictEqual(
            await db.sortedSetScore("users:postcount", oldUid),
            0,
        );
        assert.strictEqual(
            await db.sortedSetScore("users:postcount", newUid),
            2,
        );
    ...
    describe("voting", () => {
        ...
        it("should get voters", (done) => {
            socketPosts.getVoters(
                ...
                (err, data) => {
                    ...
                    assert.equal(data.upvoters[0].username, "upvoter-student");
                    done();
                },
            );
        });
        ...
        it("should get upvoters", (done) => {
            socketPosts.getUpvoters(
                ...
                (err, data) => {
                    ...
                    assert.equal(data[0].usernames, "upvoter-student");
                    done();
                },
            );
        });
        ...
        it("should prevent downvoting target user more than total daily limit", async () => {
            ...
            assert.equal(
                err.message,
                "[[error:too-many-downvotes-today-user, 1]]",
            );
    ...
    describe("bookmarking", () => {
        it("should bookmark a post", async () => {
            ...
            const hasBookmarked = await posts.hasBookmarked(
                postData.pid,
                voterUid,
            );
    ...
    describe("delete/restore/purge", () => {
        ...
        it("should delete a post", async () => {
            await apiPosts.delete(
                { uid: voterUid },
                { pid: replyPid, tid: tid },
            );
        ...
        it("should not see post content if global mod does not have posts:view_deleted privilege", (done) => {
            async.waterfall(
                [
                    function (next) {
                        user.create(
                            { username: "global mod", password: "123456" },
                            next,
                        );
                    },
                    function (uid, next) {
        ...
        it("should delete topic if last main post is deleted", async () => {
            ...
            const deleted = await topics.getTopicField(
                data.topicData.tid,
                "deleted",
            );
    ...
    describe("edit", () => {
        ...
        it("should error if title is too long", async () => {
            const longTitle = new Array(
                meta.config.maximumTitleLength + 2,
            ).join("a");
            try {
                await apiPosts.edit(
                    { uid: voterUid },
        ...
        it("should error if content is too long", async () => {
            const longContent = new Array(
                meta.config.maximumPostLength + 2,
            ).join("a");
            try {
                await apiPosts.edit(
                    { uid: voterUid },
        ...
        it("should disallow post editing for new users if post was made past the threshold for editing", async () => {
            ...
            try {
                ...
                assert.equal(
                    err.message,
                    "[[error:post-edit-duration-expired, 1]]",
                );
    ...
    describe("parse", () => {
        ...
        it("should turn relative links in post body to absolute urls", (done) => {
            ...
            const parsedContent = posts.relativeToAbsolute(
                content,
                posts.urlRegex,
            );
            assert.equal(
                ...
                `<a href="${nconf.get(
                    "base_url",
                )}/users">test</a> <a href="//youtube.com">youtube</a>`,
            );
            done();
        });
        ...
        it("should turn relative links in post body to absolute urls", (done) => {
            ...
            let parsedContent = posts.relativeToAbsolute(
                content,
                posts.urlRegex,
            );
            parsedContent = posts.relativeToAbsolute(
                parsedContent,
                posts.imgRegex,
            );
            assert.equal(
                ...
                `<a href="${nconf.get(
                    "base_url",
                )}/users">test</a> <a href="//youtube.com">youtube</a> some test <img src="${nconf.get(
                    "base_url",
                )}/path/to/img"/>`,
            );
            done();
    ...
    describe("post queue", () => {
            ...
            helpers.loginUser("globalmod", "globalmodpwd", (err, data) => {
                ...
                request(
                    ...
                    (err, res, body) => {
                        ...
                        assert.equal(
                            body.posts[0].data.content,
                            "queued topic content",
                        );
                        assert.equal(body.posts[1].type, "reply");
                        assert.equal(
                            body.posts[1].data.content,
                            "this is a queued reply",
                        );
                        done();
                    },
                );
        ...
        it("should update queued post's topic if target topic is merged", async () => {
            ...
            assert.strictEqual(
                postData[0].data.content,
                "the moved queued post",
            );
        ...
        describe(".syncBacklinks()", () => {
            ...
            it("should create a backlink if it detects a topic link in a post", async () => {
                const count = await topics.syncBacklinks({
                    pid: 2,
                    content: `This is a link to [topic 1](${nconf.get(
                        "url",
                    )}/topic/1/abcdef)`,
                });
                const events = await topics.events.get(1, 1);
                const backlinks =
                    await db.getSortedSetMembers("pid:2:backlinks");
        ...
        describe("integration tests", () => {
            it("should create a topic event in the referenced topic", async () => {
                const topic = await topics.post({
                    ...
                    content: `Some text here for the OP &ndash; ${nconf.get(
                        "url",
                    )}/topic/${tid1}`,
                });
            ...
            it("should not create a topic event if referenced topic is the same as current topic", async () => {
                await topics.reply({
                    uid: 1,
                    tid: tid1,
                    content: `Referencing itself &ndash; ${nconf.get(
                        "url",
                    )}/topic/${tid1}`,
                });
