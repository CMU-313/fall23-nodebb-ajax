...
describe("Messaging Library", () => {
    ...
    before(async () => {
        ...
        ({ jar: mocks.users.foo.jar, csrf_token: mocks.users.foo.csrf } =
            await util.promisify(helpers.loginUser)("foo", "barbar"));
        ({ jar: mocks.users.bar.jar, csrf_token: mocks.users.bar.csrf } =
            await util.promisify(helpers.loginUser)("bar", "bazbaz"));
        ({ jar: mocks.users.baz.jar, csrf_token: mocks.users.baz.csrf } =
            await util.promisify(helpers.loginUser)("baz", "quuxquux"));
        ({ jar: mocks.users.herp.jar, csrf_token: mocks.users.herp.csrf } =
            await util.promisify(helpers.loginUser)("herp", "derpderp"));
    ...
    describe("rooms", () => {
        ...
        it("should create a new chat room", async () => {
            ...
            await util.promisify(socketModules.chats.canMessage)(
        ...
        it("should fail to add user to room with invalid data", async () => {
            ...
            ({ statusCode, body } = await callv3API(
                "post",
                `/chats/${roomId}/users`,
                { uids: [null] },
                "foo",
        ...
        it("should change owner when owner leaves room", async () => {
            ...
            await util.promisify(socketModules.chats.leave)(
                { uid: mocks.users.herp.uid },
                body.response.roomId,
            );
        ...
        it("should change owner if owner is deleted", async () => {
            ...
            const { jar: senderJar, csrf_token: senderCsrf } =
                await util.promisify(helpers.loginUser)(
                    "deleted_chat_user",
                    "barbar",
                );
        ...
        it("should remove user from room", async () => {
            ...
            let isInRoom = await Messaging.isUserInRoom(
                mocks.users.herp.uid,
                roomId,
            );
            assert(isInRoom);
            ...
            isInRoom = await Messaging.isUserInRoom(
                mocks.users.herp.uid,
                roomId,
            );
            assert(!isInRoom);
        ...
        it("should return not allowed error if mid is not in room", async () => {
            ...
            try {
                await util.promisify(socketModules.chats.getRaw)(
        ...
        it("should notify offline users of message", async () => {
            ...
            assert.strictEqual(
                ...
                "[[notifications:new_message_from, foo-student]]",
            );
            assert.strictEqual(
                notification.nid,
                `chat_${mocks.users.foo.uid}_${roomId}`,
            );
            assert.strictEqual(
                notification.path,
        ...
        it("should mark all rooms read", (done) => {
            socketModules.chats.markAllRead(
                { uid: mocks.users.foo.uid },
                {},
                (err) => {
                    assert.ifError(err);
        ...
        it("should fail to rename room with invalid data", async () => {
            ...
            assert.strictEqual(
                body.status.message,
                await translator.translate(
                    "[[error:required-parameters-missing, name]]",
                ),
            );
        });

        it("should rename room", async () => {
            const { statusCode } = await callv3API(
                "put",
                `/chats/${roomId}`,
                { name: "new room name" },
                "foo",
        ...
        it("should return true if user is dnd", (done) => {
            db.setObjectField(
                `user:${mocks.users.herp.uid}`,
                "status",
                "dnd",
                (err) => {
                    assert.ifError(err);
                    socketModules.chats.isDnD(
                        { uid: mocks.users.foo.uid },
                        mocks.users.herp.uid,
                        (err, isDnD) => {
                            assert.ifError(err);
        ...
        it("should fail to check if user has private chat with invalid data", (done) => {
            socketModules.chats.hasPrivateChat({ uid: null }, null, (err) => {
                assert.equal(err.message, "[[error:invalid-data]]");
                socketModules.chats.hasPrivateChat(
                    { uid: mocks.users.foo.uid },
                    null,
                    (err) => {
                        assert.equal(err.message, "[[error:invalid-data]]");
    ...
    describe("edit/delete", () => {
        ...
        it("should fail to delete message if not owner", (done) => {
            socketModules.chats.delete(
                { uid: mocks.users.herp.uid },
                { messageId: mid, roomId: roomId },
                (err) => {
                    assert.equal(
                        err.message,
                        "[[error:cant-delete-chat-message]]",
                    );
                    done();
                },
            );
            ...
            socketModules.chats.getMessages(
                ...
                (err, messages) => {
                    ...
                    messages.forEach((msg) => {
                        ...
                        assert(
                            !msg.deleted || msg.cleanedContent,
                            "[[modules:chat.message-deleted]]",
                            msg.content,
                        );
                    });
        ...
        describe("disabled via ACP", () => {
            ...
            it("should error out for regular users", async () => {
                try {
                    await socketModules.chats.delete(
                        { uid: mocks.users.baz.uid },
                        { messageId: mid2, roomId: roomId },
                    );
                } catch (err) {
                    assert.strictEqual(
                        "[[error:chat-message-editing-disabled]]",
                        err.message,
                    );
                }
            });
            ...
            it("should succeed for administrators", async () => {
                ...
                await socketModules.chats.restore(
                    { uid: mocks.users.foo.uid },
                    { messageId: mid2, roomId: roomId },
                );
            });
    ...
    describe("logged in chat controller", () => {
        ...
        it("should return 404 if user is not in room", async () => {
            const data = await util.promisify(helpers.loginUser)(
                "baz",
                "quuxquux",
            );
            const response = await request(
