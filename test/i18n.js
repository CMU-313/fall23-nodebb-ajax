                ...
                it('translations should contain every language file contained in the source language directory', () => {
                    sourceFiles.forEach((relativePath) => {
                        assert(
                            ...
                            `${relativePath.slice(
                                1
                            )} was found in source files but was not found in language "${language}" (likely not internationalized)`
                        );
                    });
                });
                ...
                it('should not contain any extraneous files not included in the source language directory', () => {
                    files.forEach((relativePath) => {
                        assert(
                            ...
                            `${relativePath.slice(
                                1
                            )} was found in language "${language}" but there is no source file for it (likely removed from en-GB)`
                        );
                    });
                });
            });
                        ...
                        sourceKeys.forEach((key) => {
                            assert(
                                ...
                                `${namespace.slice(
                                    1,
                                    -5
                                )}:${key} missing in ${language}`
                            );
                        });
                        assert.strictEqual(
                            sourceKeys.length,
                            translationKeys.length,
                            `Extra keys found in namespace ${namespace.slice(
                                1,
                                -5
