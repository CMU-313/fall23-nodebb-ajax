...
describe('Sorted Set methods', () => {
    ...
    describe('sortedSetAddMulti()', () => {
        ...
        it('should error if score is null', async () => {
            ...
            assert.equal(err.message, '[[error:invalid-score, null]]');
            assert.strictEqual(
                await db.isSortedSetMember('bulk4', 'dontadd'),
                false
            );
            assert.strictEqual(
                await db.isSortedSetMember('bulk5', 'dontadd'),
                false
            );
        });
    });
    ...
    describe('sortedSetRemove()', () => {
        ...
        it('should remove value from multiple keys', async () => {
            ...
            assert.deepStrictEqual(
                await db.getSortedSetRange('multiTest3', 0, -1),
                ['one', 'two', 'four']
            );
            assert.deepStrictEqual(
                await db.getSortedSetRange('multiTest4', 0, -1),
                ['four', 'five', 'six']
            );
        });
