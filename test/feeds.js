    ...
    describe('private feeds and tokens', () => {
                    ...
                    async.parallel(
                        {
                            ...
                            test2: function (next) {
                                request(
                                    `${nconf.get(
                                        'url'
                                    )}/category/${cid}.rss?token=sometoken`,
                                    {},
                                    next
                                );
                            },
                        ...
                        (err, results) => {
                            ...
                            assert(
                                results.test1[0].body.includes(
                                    'Login to your account'
                                )
                            );
                            assert(
                                results.test2[0].body.includes(
                                    'Login to your account'
                                )
                            );
                            done();
        ...
        it('should not allow access if token is wrong', (done) => {
            request(
                `${nconf.get(
                    'url'
                )}/category/${cid}.rss?uid=${fooUid}&token=sometoken`,
                {},
                (err, res, body) => {
                    assert.ifError(err);
                    assert.equal(res.statusCode, 200);
            ...
            request(
                ...
                (err, res, body) => {
                    ...
                    request(
                        `${nconf.get(
                            'url'
                        )}/category/${cid}.rss?uid=${fooUid}&token=${rssToken}`,
                        {},
                        (err, res, body) => {
                            assert.ifError(err);
                            assert.equal(res.statusCode, 200);
